...
//rest - (при создании функции) остаточные параметры, добавляется как последний аргумент в функции,
// и собирает все последующие параметры в массив.
function showrest(arg1, arg2, ...arg) {
    for (let i of arg) {
        console.log(i)
    }
}

//spread - разбивает массив на параметры, при вызове функции
let arr = [1, 2, 3]
showrest(...arr) // showrest(store[0], store[1], store[2])

// чем отличается arguments от [...arr], тем что arguments не имеет методов массива
let arr2 = [0, ...arr]  // создание массивов

//rest работает с итерируемыми объектами (строки, массивы, map, set)
...
//Д - это специальный синтаксис, который позволяет распаковать массив, объект на переменные.
//Работает с массивами и объектами

arr = ['Ilya', 'Stas']
let [first, second] = arr
console.log(first) // Ilya

//Работает с любым перебираемым объектом (string, array, map, set)
//Можно пропускать елементы:
const [firstone, , therd] = [1, 2, 3]
//Работает с методами возвращающими массив:
const [one, secondTwo] = 'Hello world'.split(' ')
//Можно присвоить чему угодно, хот свойству объекта:
let obj = {}
[obj.name, obj.age] = ['Stas', 36]
//Можно собрать остальные элементы в массив:
let [name1, name2, ...rest] = ["Julius", "Caesar", "Consul", "of the Roman Republic"];
//Если массив пустой [], то переменные будут undefined
//Значения поумолчанию: 
let [nameOne = 'Stas', ageOne = 20] = ['Ann']
